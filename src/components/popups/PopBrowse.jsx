...
function PopBrowse() {
  ...
  const clickEditTask = async () => {
    ...
    await editTasks({ ...taskData, id, token: user?.token })
      .then((response) => {
        setCards(response.tasks);
        navigate(paths.MAIN);
      })
  ...
  switch (indexCard?.topic) {
    case "Research":
      background = theme
        ? lightTheme.backgroundGreen
        : darkTheme.backgroundGreen;
      color = theme ? lightTheme.colorGreen : darkTheme.colorGreen;
      break;
    case "Web Design":
      background = theme
        ? lightTheme.backgroundOrange
        : darkTheme.backgroundOrange;
      color = theme ? lightTheme.colorOrange : darkTheme.colorOrange;
      break;
    case "Copywriting":
      background = theme
        ? lightTheme.backgroundPurple
        : darkTheme.backgroundPurple;
    ...
    <S.PopBrowse id="popBrowse">
      <S.PopBrowseContainer>
        <S.PopBrowseBlock>
          <S.PopBrowseContent>
            <S.PopBrowseTopBlock>
              <S.PopBrowseTtl>{indexCard?.title}</S.PopBrowseTtl>
                  ...
                  <>
                    {statusList.map((status, index) => (
                      <S.StatusTheme
                        ...
                        className={status === statusTask ? "grey" : ""}
                        $editPage={editPage}
                      >
                        <S.StatusThemeP
                          className={status === statusTask ? "grey" : ""}
            ...
            <S.PopBrowseWrap>
                  ...
                  <S.FormBrowseArea
                    style={
                      !editPage
                        ? {
                            background: theme
                              ? lightTheme.readOnlyBg
                              : darkTheme.readOnlyBg,
                          }
                        : null
                    }
                    name="text"
              ...
              <S.PopBrowseWrap2>
                <S.StatusPSubttl2>Даты</S.StatusPSubttl2>
                <Calendar
            ...
            {!editPage ? (
              <S.PopBrowseBtnBrowse>
                <S.BtnGroup>
                  ...
                  <S.BtnBor onClick={clickDeleteTask}>Удалить задачу</S.BtnBor>
                </S.BtnGroup>
                <Link to={paths.MAIN}>
              ...
              <S.PopBrowseBtnEdit>
                <S.BtnGroup>
                  <S.BtnBg onClick={clickEditTask}>Сохранить</S.BtnBg>
